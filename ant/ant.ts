...
const decodeMessageType = (messageId) => {
	switch (messageId) {
		case Constants.MESSAGE_STARTUP:	return "MESSAGE_STARTUP";
		case Constants.MESSAGE_STARTUP:	return "MESSAGE_STARTUP";
		case Constants.MESSAGE_STARTUP:	return "MESSAGE_STARTUP";
		case Constants.MESSAGE_STARTUP:	return "MESSAGE_STARTUP";
		
		// Data messages
		case Constants.MESSAGE_CHANNEL_BROADCAST_DATA:	return "MESSAGE_CHANNEL_BROADCAST_DATA";
		case Constants.MESSAGE_CHANNEL_ACKNOWLEDGED_DATA:	return "MESSAGE_CHANNEL_ACKNOWLEDGED_DATA";
		case Constants.MESSAGE_CHANNEL_BURST_DATA:	return "MESSAGE_CHANNEL_BURST_DATA";
	
		// Channel event messages
		case Constants.MESSAGE_CHANNEL_EVENT:	return "MESSAGE_CHANNEL_EVENT";
		case Constants.CAPABILITIES_PROX_SEARCH_ENABLED:	return "CAPABILITIES_PROX_SEARCH_ENABLED";
		case Constants.CAPABILITIES_EXT_ASSIGN_ENABLED:	return "CAPABILITIES_EXT_ASSIGN_ENABLED";
		case Constants.CAPABILITIES_FS_ANTFS_ENABLED:	return "CAPABILITIES_FS_ANTFS_ENABLED";
		default:
		return "?????????????";
	}
}
...
export class USBDriver extends events.EventEmitter {
	...
	public read(data: Buffer) {
		var copy = Buffer.from(data);

		const messageID = data.readUInt8(2);
		console.log('DATA RECV: ', decodeMessageType(messageID), copy);
...
export abstract class BaseSensor extends events.EventEmitter {
	...
	protected write(data: Buffer, debug?: string) {
...
// 	MESSAGE_CHANNEL_FREQUENCY = 0x45,
// 	MESSAGE_CHANNEL_TX_POWER = 0x60,
// 	MESSAGE_NETWORK_KEY = 0x46,MESSAGE_TX_POWER = 0x47,
// 	MESSAGE_PROXIMITY_SEARCH = 0x71,
// 	MESSAGE_ENABLE_RX_EXT = 0x66,
// 	MESSAGE_LIB_CONFIG = 0x6E,
// 	MESSAGE_CHANNEL_OPEN_RX_SCAN = 0x5B,
...
// 		this.maxChannels = 0;
// 		this.usedChannels = 0;
// 		this.write(Messages.resetSystem(), 'reset');
// 	}
...
// 	write(data: Buffer, source: string) {
// 		console.log('DATA SEND: ', source, data);
// 		this.outEp.transfer(data, (error) => {
// 			if (error) {
// 				console.log('ERROR SEND: ', error);
// 			}
...
// 	read(data: Buffer) {
// 		const messageID = data.readUInt8(2);
// 		console.log('DATA RECV: ',  messageID, decodeMessageType(messageID), data);
// 		if (messageID === Constants.MESSAGE_STARTUP) {
// 			this.write(Messages.requestMessage(0, Constants.MESSAGE_CAPABILITIES), 'read');
// 		} else if (messageID === Constants.MESSAGE_CAPABILITIES) {
// 			this.maxChannels = data.readUInt8(3);
// 			this.canScan = (data.readUInt8(7) & 0x06) === 0x06;
// 			this.write(Messages.setNetworkKey(), 'read');
// 		} else if (messageID === Constants.MESSAGE_CHANNEL_EVENT && data.readUInt8(4) === Constants.MESSAGE_NETWORK_KEY) {
// 			this.emit('startup', data);
// 		} else {
// 			this.emit('read', data);
...
// 					switch (status.code) {
// 						case Constants.EVENT_CHANNEL_CLOSED:
// 							this.write(Messages.unassignChannel(channel), 'scan');
// 							break;
// 						default:
// 							break;
// 					}
// 					break;
// 				case Constants.MESSAGE_CHANNEL_ASSIGN:
// 					this.write(Messages.setDevice(channel, 0, 0, 0), 'scan');
// 					break;
// 				case Constants.MESSAGE_CHANNEL_ID:
// 					this.write(Messages.setFrequency(channel, frequency), 'scan');
// 					break;
// 				case Constants.MESSAGE_CHANNEL_FREQUENCY:
// 					this.write(Messages.setRxExt(), 'scan');
// 					break;
// 				case Constants.MESSAGE_ENABLE_RX_EXT:
// 					this.write(Messages.libConfig(channel, 0xE0), 'scan');
// 					break;
// 				case Constants.MESSAGE_LIB_CONFIG:
// 					this.write(Messages.openRxScan(), 'scan');
// 					break;
// 				case Constants.MESSAGE_CHANNEL_OPEN_RX_SCAN:
...
// 				this.stick.on('read', this.decodeDataCbk);
// 			}
// 			process.nextTick(() => this.emit('scan'));
// 		} else if (this.stick.attach(this, true)) {
// 			this.channel = channel;
// 			this.deviceID = 0;
// 			this.transmissionType = 0;
...
// 			this.write(Messages.assignChannel(channel, type), 'scan');
// 		} else {
// 			throw 'cannot attach';
// 		}
// 	}
...
// 					switch (status.code) {
// 						case Constants.EVENT_CHANNEL_CLOSED:
// 							this.write(Messages.unassignChannel(channel), 'attach EVENT_CHANNEL_CLOSED');
// 							break;
// 						default:
// 							break;
// 					}
// 					break;
// 				case Constants.MESSAGE_CHANNEL_ASSIGN:
// 					this.write(Messages.setDevice(channel, deviceID, deviceType, transmissionType), `attach MESSAGE_CHANNEL_ASSIGN ${channel} ${deviceID} ${deviceType} ${transmissionType}`);
// 					break;
// 				case Constants.MESSAGE_CHANNEL_ID:
// 					this.write(Messages.searchChannel(channel, timeout), 'attach MESSAGE_CHANNEL_ID ' + channel);
// 					break;
// 				case Constants.MESSAGE_CHANNEL_SEARCH_TIMEOUT:
// 					this.write(Messages.setFrequency(channel, frequency), 'attach MESSAGE_CHANNEL_SEARCH_TIMEOUT');
// 					break;
// 				case Constants.MESSAGE_CHANNEL_FREQUENCY:
// 					this.write(Messages.setPeriod(channel, period), 'attach MESSAGE_CHANNEL_FREQUENCY');
// 					break;
// 				case Constants.MESSAGE_CHANNEL_PERIOD:
// 					this.write(Messages.libConfig(channel, 0xE0), 'attach MESSAGE_CHANNEL_PERIOD');
// 					break;
// 				case Constants.MESSAGE_LIB_CONFIG:
// 					this.write(Messages.openChannel(channel), 'attach MESSAGE_LIB_CONFIG');
// 					break;
// 				case Constants.MESSAGE_CHANNEL_OPEN:
// 					if (this.decodeDataCbk) {
// 						this.stick.on('read', this.decodeDataCbk);
...
// 		this.write(Messages.assignChannel(channel, type), 'attach assignChannel');
// 	}
...
// 			return;
// 		}
// 		this.write(Messages.closeChannel(this.channel), 'detach');
// 		if (!this.stick.detach(this)) {
// 			throw 'error detaching';
// 		}
// 	}

// 	protected write(data: Buffer, source: string) {
// 		this.stick.write(data, source);
// 	}
