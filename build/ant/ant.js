...
var Messages = /** @class */ (function () {
    ...
    Messages.acknowledgedData = function (payload) {
        return Messages.buildMessage(payload, Constants.MESSAGE_CHANNEL_ACKNOWLEDGED_DATA);
    };
    Messages.broadcastData = function (payload) {
...
var USBDriver = /** @class */ (function (_super) {
    ...
    USBDriver.prototype.blockDevice = function (device) {
        USBDriver.deviceInUse.push(device);
    };
    USBDriver.prototype.unblockDevice = function (device) {
        var deviceIdx = undefined;
        for (var i = 0; i < USBDriver.deviceInUse.length; i++) {
            var blocked = USBDriver.deviceInUse[i];
    ...
    USBDriver.prototype.open = function () {
        ...
        this.inEp.on('data', function (data) {
            ...
            if (data.readUInt8(0) !== 0xA4) {
                _this.emit('error', { error: 'SYNC missing', context: data });
            }
            var len = data.length;
            var beginBlock = 0;
    ...
    USBDriver.prototype.write = function (data, source) {
    ...
    USBDriver.prototype.read = function (data) {
        ...
        console.log('DATA RECV: ', messageID, data);
...
var BaseSensor = /** @class */ (function (_super) {
    ...
    BaseSensor.prototype.scan = function (type, frequency) {
        ...
        var onStatus = function (status) {
            switch (status.msg) {
                case 0x01:
                    switch (status.code) {
                        case Constants.EVENT_CHANNEL_CLOSED:
                            _this.write(Messages.unassignChannel(channel), 'scan');
                            break;
                        default:
                            break;
                    }
                ...
                case Constants.MESSAGE_CHANNEL_ASSIGN:
                    _this.write(Messages.setDevice(channel, 0, 0, 0), 'scan');
                    break;
                case Constants.MESSAGE_CHANNEL_ID:
                    _this.write(Messages.setFrequency(channel, frequency), 'scan');
                    break;
                case Constants.MESSAGE_CHANNEL_FREQUENCY:
                    _this.write(Messages.setRxExt(), 'scan');
                    break;
                case Constants.MESSAGE_ENABLE_RX_EXT:
                    _this.write(Messages.libConfig(channel, 0xE0), 'scan');
                    break;
                case Constants.MESSAGE_LIB_CONFIG:
                    _this.write(Messages.openRxScan(), 'scan');
                    break;
        ...
        if (this.stick.isScanning()) {
            ...
            process.nextTick(function () { return _this.emit('scan'); });
        }
        ...
        var onStatus = function (status) {
            switch (status.msg) {
                case 0x01:
                    switch (status.code) {
                        case Constants.EVENT_CHANNEL_CLOSED:
                            _this.write(Messages.unassignChannel(channel), 'attach EVENT_CHANNEL_CLOSED');
                            break;
                        default:
                            break;
                    }
                    break;
                case Constants.MESSAGE_CHANNEL_ASSIGN:
                    _this.write(Messages.setDevice(channel, deviceID, deviceType, transmissionType), "attach MESSAGE_CHANNEL_ASSIGN " + channel + " " + deviceID + " " + deviceType + " " + transmissionType);
                    break;
                case Constants.MESSAGE_CHANNEL_ID:
                    _this.write(Messages.searchChannel(channel, timeout), 'attach MESSAGE_CHANNEL_ID ' + channel);
                    break;
                case Constants.MESSAGE_CHANNEL_SEARCH_TIMEOUT:
                    _this.write(Messages.setFrequency(channel, frequency), 'attach MESSAGE_CHANNEL_SEARCH_TIMEOUT');
                    break;
                case Constants.MESSAGE_CHANNEL_FREQUENCY:
                    _this.write(Messages.setPeriod(channel, period), 'attach MESSAGE_CHANNEL_FREQUENCY');
                    break;
                case Constants.MESSAGE_CHANNEL_PERIOD:
                    _this.write(Messages.libConfig(channel, 0xE0), 'attach MESSAGE_CHANNEL_PERIOD');
                    break;
                case Constants.MESSAGE_LIB_CONFIG:
                    _this.write(Messages.openChannel(channel), 'attach MESSAGE_LIB_CONFIG');
                    break;
    ...
    BaseSensor.prototype.write = function (data, source) {
